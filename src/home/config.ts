...
/**
 * Custom homepage config for `/home/config`.
 * Converted directly from your verified working template.
 */
...
type HomeBlockTemplate = Omit<HomeConfigMediaBlock, "title"> & {
...
// 🌟 核心：原汁原味保留你 100% 能读取的全部周更表与 R2 分类列表
...
function resolveMediaBlock(
	...
	language: string,
): HomeConfigMediaBlock {
	const { titleKey, ...rest } = block;
	if (!titleKey) return rest as HomeConfigMediaBlock;
...
// 🌟 导出函数名改为匹配服务端的自定义标准：createDefaultHomeConfig
export async function createDefaultHomeConfig(
	...
	for (const section of createV2BlockTemplates(
		...
		blocks.push(resolveMediaBlock(section as HomeBlockTemplate, options.language) as HomeConfigBlock);
	}
	...
	return {
		...
		carouselSourceId: "tmdb_popular_movies", // 保留你原版定义的轮播源
		blocks,
	};
}
